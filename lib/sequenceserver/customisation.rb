     ...
     def construct_custom_sequence_hyperlink(options)
       ...
       if matches #if the sequence_id conforms to our expectations
         ...
         if geneModelMatch
           ...
           return "http://amborella.uga.edu/mgb2/gbrowse/amborella_amtr_v_0_10/?name=#{matches[1]}:#{from}..#{to}"
           #return "http://amborella.uga.edu/mgb2/gbrowse/amborella_amtr_v_0_10/?name=#{matches[1]}:#{options[:hit_coordinates][0]}..#{options[:hit_coordinates][1]}"
         end
       else
